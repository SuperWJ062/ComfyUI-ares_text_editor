...
A ComfyUI custom node that **pauses the workflow for manual text editing** before continuing.
...
- **保留编辑内容**：已编辑的文本不会被覆盖；通过「Load New Input」可清空编辑框以重新获取输入
- **单次通过**：点击「Confirm & Continue」提交后工作流只继续执行一次，下游不会重复运行
- **本地 API**：通过 ComfyUI 内置服务器提供接口，无任何外部网络请求

## 节点

### Ares Text Editor (`TextEditorWithConfirm`)
...
3. 在编辑框中修改文本
4. 点击 **Confirm & Continue**，工作流携带编辑后的文本继续执行下游节点
...
- 点击「Confirm & Continue」后，前端将编辑内容 POST 到服务器，后端解除阻塞并返回该文本
...
- **节点一直显示执行中？** 说明节点正在等待编辑确认，请在编辑框中确认内容后点击「Confirm & Continue」。
